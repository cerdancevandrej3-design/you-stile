// Regenerate worst catalog nails into public/nails/_regen_batch1/ (same filenames)
import fs from "fs";
...
const OUT_DIR = path.join(__dirname, "public", "nails", "_regen_batch1");
const PROMPTS = path.join(__dirname, "public", "nails", "_regen_batch1_prompts.json");
...
Dorsal TOP-DOWN. Back of the hand to the camera. Camera ABOVE, looking down onto the TOP of 4-5 nail plates. Hand resting on draped cream satin. Fingers slightly together. Thumb nail also from the TOP of the plate.
Looks like t_0264e003188b1e32 / t_4e5f9035d996c7ea: real skin pores, real knuckles, no jewelry.

Lighting: bright salon softbox, wet high-gloss gel top coat, a crisp vertical white specular highlight down each nail plate. The manicure is the hero.

FORBIDDEN: palm to camera, side profile, 3/4 sidewall, nail edge/torus visible, rings on fingers, bracelets, waxy doll skin, extra fingers, floating jewelry, product bottles, Amazon packshot, orchids, pearls as props.
`;
...
async function main() {
  ...
  for (const item of items) {
    if (only.length && !only.includes(item.filename) && !only.includes(item.id)) continue;
    const dest = path.join(OUT_DIR, item.filename);
    if (fs.existsSync(dest) && fs.statSync(dest).size > 20000 && !only.length) {
      console.log("[SKIP]", item.filename);
      continue;
    }
    console.log("[GEN]", item.filename);
    try {
      let last = null;
      for (let a = 1; a <= 2; a++) {
        try {
          ...
          last = e;
          console.log("[RETRY]", item.filename, a, String(e.message || e).slice(0, 120));
          await sleep(4000);
        }
      }
      if (last) throw last;
      console.log("[OK]", item.filename, fs.statSync(dest).size);
    } catch (e) {
      console.error("[FAIL]", item.filename, String(e.message || e).slice(0, 180));
