...
const RESULTS_TTL_PAID_MS = 30 * 24 * 60 * 60 * 1000; // 30 дней — после оплаты / промо / с профилем
const UNFINISHED_ORDER_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours
/** После оплаты всегда долгое хранение; имя/visitor — тоже. */
...
interface OrderRecord {
  ...
  visitorId?: string;
  userName?: string;
  /** Нормализованный телефон 7XXXXXXXXXX — ключ кабинета «Мои образы» */
  phone?: string;
}
...
const ANALYSIS_MODEL = "google/gemini-3.5-flash-lite";
...
/** Короткий SCENE+LIGHT для GPT Image 2: место + формула света (интенсивность, температура, направление, тень). */
function getOccasionAtmosphere(wishes: string, idx: number = 0): string {
  ...
  if (w.includes("пляж") || w.includes("отдых") || w.includes("курорт") || w.includes("яхта"))
    return pick([
      " SCENE: white terrace overlooking the sea. LIGHT: warm golden-hour sun from upper right, soft long shadow left, saturated but natural colors.",
      " SCENE: beach club, turquoise water behind. LIGHT: soft midday sun from above, even fill, minimal harsh shadows.",
      " SCENE: luxury yacht deck at sunset. LIGHT: warm amber key from camera-left 45°, golden reflections on water, gentle rim on hair.",
    ]);
  if (w.includes("свидание") || w.includes("романтич"))
    return pick([
      " SCENE: intimate candlelit restaurant, soft bokeh. LIGHT: warm tungsten key from table candles below-front, soft falloff, cozy shadows.",
      " SCENE: rooftop terrace at sunset, city lights below. LIGHT: warm backlight from horizon, soft fill on face from front-right.",
      " SCENE: cozy wine bar, brick walls. LIGHT: warm amber wall sconces from sides, soft shadow under chin.",
    ]);
  if (w.includes("ресторан"))
    return pick([
      " SCENE: upscale restaurant, marble table, chandelier. LIGHT: warm ambient overhead + soft side fill, realistic indoor contrast.",
      " SCENE: outdoor restaurant terrace, greenery. LIGHT: golden evening sun from upper left, dappled soft shadows.",
      " SCENE: modern fine-dining room. LIGHT: focused warm spotlight from above-front, deep soft background.",
    ]);
  if (w.includes("вечеринк") || w.includes("клуб") || w.includes("ночная"))
    return pick([
      " SCENE: upscale nightclub interior. LIGHT: cool neon magenta/cyan from sides, dark ambient, face still clearly lit.",
      " SCENE: rooftop night party, skyline. LIGHT: warm string lights + soft frontal fill, city glow behind.",
      " SCENE: exclusive dark lounge. LIGHT: single soft key from above-left 45°, moody shadows, readable face.",
    ]);
  if (w.includes("свадьб") || w.includes("торжеств") || w.includes("выпускн"))
    return pick([
      " SCENE: grand ballroom, crystal chandeliers. LIGHT: soft warm overhead diffusion, gentle fill, elegant glow on fabric.",
      " SCENE: garden ceremony, trees. LIGHT: soft natural daylight through leaves, cool-green ambient, soft facial shadows.",
      " SCENE: historic venue exterior, columns. LIGHT: warm golden-hour key from camera-right, long soft shadow left.",
    ]);
  if (w.includes("офис") || w.includes("деловая") || w.includes("бизнес"))
    return pick([
      " SCENE: modern glass office, city through windows. LIGHT: cool daylight from large windows camera-left, soft indoor fill.",
      " SCENE: business district street, glass buildings. LIGHT: bright overcast daylight, even soft shadows.",
      " SCENE: minimalist conference room. LIGHT: soft diffused overhead panels, clean neutral color, low contrast.",
    ]);
  if (w.includes("спорт") || w.includes("фитнес"))
    return pick([
      " SCENE: modern premium gym. LIGHT: cool bright overhead LEDs, slight side contrast, sharp fabric texture.",
      " SCENE: city park at sunrise. LIGHT: warm low sun from left, long soft shadows, fresh air look.",
      " SCENE: rooftop workout space, skyline. LIGHT: clear midday sun from above, crisp shadows, high clarity.",
    ]);
  if (w.includes("прогулк") || w.includes("кафе"))
    return pick([
      " SCENE: European cobblestone street. LIGHT: soft morning daylight from upper left, natural urban color.",
      " SCENE: outdoor café terrace. LIGHT: dappled sun through trees, warm highlights, soft green bokeh.",
      " SCENE: city park path. LIGHT: golden afternoon sun from right, soft leaf shadows on ground.",
    ]);
  if (w.includes("театр") || w.includes("выставк"))
    return pick([
      " SCENE: grand theatre lobby. LIGHT: warm chandelier overhead, soft gold reflections on fabric.",
      " SCENE: white-wall art gallery. LIGHT: clean museum spots from above, neutral white balance, sharp detail.",
      " SCENE: theatre entrance at night. LIGHT: cool marquee glow + warm frontal fill on face.",
    ]);
  if (w.includes("путешеств") || w.includes("самолёт"))
    return pick([
      " SCENE: modern airport terminal windows. LIGHT: bright cool daylight from large glass, soft ambient fill.",
      " SCENE: iconic city landmark outdoors. LIGHT: clear natural daylight, vivid but realistic colors.",
      " SCENE: boutique hotel lobby. LIGHT: warm soft interior lamps from sides, polished materials.",
    ]);
  if (w.includes("фотосессия"))
    return pick([
      " SCENE: luxury penthouse, skyline windows. LIGHT: strong side key from window left 45°, soft fill right, editorial contrast.",
      " SCENE: urban street, architecture behind. LIGHT: natural daylight, slight backlight rim, realistic street color.",
      " SCENE: rooftop terrace, city panorama. LIGHT: warm golden-hour key from right, soft long shadows.",
    ]);
  if (w.includes("фестиваль") || w.includes("концерт"))
    return pick([
      " SCENE: outdoor festival field at sunset. LIGHT: warm low sun backlight, soft frontal fill, vivid natural color.",
      " SCENE: concert venue. LIGHT: dramatic stage washes in color, face kept readable with soft front fill.",
      " SCENE: festival art installation. LIGHT: warm natural late-day light, saturated but realistic palette.",
    ]);
  if (w.includes("горнолыжн"))
    return pick([
      " SCENE: alpine ski resort, snow peaks. LIGHT: crisp cold daylight, bright snow bounce fill under chin.",
      " SCENE: ski slope, powder snow. LIGHT: clear high sun, sharp shadows, cool blue ambient in shade.",
      " SCENE: mountain chalet interior, fireplace. LIGHT: warm fire glow camera-left, soft room fill.",
    ]);
  if (w.includes("загородн") || w.includes("природ"))
    return pick([
      " SCENE: countryside meadow estate. LIGHT: soft natural daylight, open sky fill, fresh greens.",
      " SCENE: forest path. LIGHT: dappled sun through trees, cool green ambient, soft facial shadow.",
      " SCENE: lakeside dock. LIGHT: golden-hour sun from right, warm water reflections, soft rim light.",
    ]);
  if (w.includes("корпоратив"))
    return pick([
      " SCENE: corporate event venue. LIGHT: warm event wash from above, soft frontal fill, realistic crowd-ready look.",
      " SCENE: hotel ballroom celebration. LIGHT: soft chandelier ambient, gentle side accent on fabric.",
      " SCENE: rooftop corporate party, city view. LIGHT: evening city glow + soft warm key on face.",
    ]);
  return [
    " SCENE: European city street, stone buildings. LIGHT: warm afternoon sun from upper left, soft urban shadows, realistic color.",
    " SCENE: modern interior, floor-to-ceiling windows. LIGHT: soft daylight from windows left, gentle indoor fill.",
    " SCENE: lush city park. LIGHT: dappled sun through trees, soft green bokeh, natural skin tones.",
  ][idx % 3];
}

/** Промпт для IMAGE_MODEL: максимум сохранения лица + вау по ткани/фону. */
function buildOutfitImagePrompt(opts: {
  ...
  const pose = isPhotoshoot
    ? "POSE: editorial full-body fashion pose, slight 45° body angle OR weight on one hip, natural hands, confident gaze toward camera."
    : "POSE: natural full-body, slight angle, weight on one leg, relaxed shoulders, chin level, gaze toward camera.";
  const bodyExtra = (opts.bodyBuildInstruction || "").trim();

  // OUTFIT сразу после лица — если сработает обрезка по лимиту, детали одежды не потеряются
  return `Edit Image 1. Image 1 is a PHOTO of a REAL person — preserve identity above everything else.

FACE IDENTITY (HIGHEST PRIORITY):
- Exact same face as Image 1: bone structure, eyes, brows, nose, lips, jaw, cheeks, forehead, ears
- Keep freckles/moles/scars/asymmetry; same age, same ${gender}, same ethnicity/skin undertone
- Same hair: color, length, hairline, parting, texture (minor tidy OK — still same hair identity)
- EXPRESSION: match Image 1 exactly — no added smile if none; same smile intensity if any
- Natural skin (pores) — NO beauty filter that changes identity, NO face swap, NO different person
- A close friend must instantly recognize this person

OUTFIT DETAILS (apply precisely):
${sanitizeEditPrompt(opts.editPrompt)}

BODY & FRAMING:
${bodyExtra ? bodyExtra + "\n" : ""}- Full body head-to-shoes, torso facing camera, correct proportions
- ${pose}

WARDROBE DETAIL (wow):
- Ultra-sharp fabric: weave, knit, leather grain, silk sheen, wool, stitching, buttons, zippers, hems
- Believable drape/folds; accurate fit; true colors; crisp accessories

SCENE & LIGHT (editorial wow):
${atmosphere}
- Face always sharp and clearly lit; catchlights in eyes; soft contact shadow under feet
- Real readable background (not muddy blur / empty gray); magazine photoreal, not plastic AI
- Single person only; no watermark/text/logo/collage/extra limbs; aspect 3:4`;
}
...
async function startServer() {
  ...
  const nailsSub = createNailsSubscription(PROJECT_ROOT);
...
updatePromoHint();
  ...
  app.get("/api/admin-visits", (req: Request, res: Response) => {
    const period = (req.query.period as string) || "all";
  ...
  app.get("/api/admin-behavior", (req: Request, res: Response) => {
    const pin = (req.query.pin || "").toString();
    if (pin !== "913260") return res.status(403).json({ error: "forbidden" });
    const period = (req.query.period as string) || "all";
    res.json(summarizePageviews(period));
  });
  ...
  app.post("/api/create-payment", async (req: Request, res: Response) => {
    try {
      ...
      // Для образов телефон обязателен — иначе не восстановить «Мои образы»
      if ((tier === "premium" || tier === "standard") && !phone) {
        return res.status(400).json({ error: "Укажите телефон — так мы сохраним ваши образы" });
      }
      const stats = loadStats();
      ...
      if (event.type === "payment.succeeded") {
        ...
        if (paymentId) {
          if (tier === "nails_month") {
            ...
            console.log(`[YooKassa Webhook] Nails month paid: ${paymentId}`);
            notifyTelegram(`✅ Оплата ${amount}₽ (База ногтей — месяц)`);
          } else if (tier === "grooming") {
            console.log(`[YooKassa Webhook] Grooming paid: ${paymentId}`);
  ...
  app.post("/api/stylize", (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      // Сезон: приоритет у явного выбора с формы, иначе — из текста пожеланий
      ...
      if (paymentId) {
        try {
          ...
          if (fs.existsSync(inputPath)) {
            ...
            writeJsonAtomic(inputPath, { ...prev, detectedGender });
          }
        } catch {}
      }
...
Проанализируй лицо на фото и верни JSON по схеме для режима ${mode}. Только крупный план (close-up), без full-body.`;
      ...
      const imageBeforeUrl = await persistGroomingImage(
        beforeFolderId,
        "before",
        `data:${mimeType};base64,${referenceImageBase64}`
      );

      // Одно преображение: новая причёска + лучшая одежда + лицо после процедур/ухода
      // Лицо — по тому же жёсткому правилу, что у основного стилиста (identity first)
      const afterPromptWrap = (editPrompt: string) =>
        `Edit Image 1. Image 1 is a PHOTO of a REAL person — this is the BEFORE reference. Preserve face identity above everything else.

FACE IDENTITY (HIGHEST PRIORITY — never break):
- Exact same face as Image 1: bone structure, eyes, brows, nose, lips, jaw, cheeks, forehead, ears
- Keep freckles/moles/scars/asymmetry; same age range, same gender, same ethnicity/skin undertone
- EXPRESSION: match Image 1 closely — no added smile if none; same smile intensity if any
- Natural skin with pores — NO beauty filter that changes identity, NO face swap, NO different person
- A close friend must instantly recognize this person as Image 1

ALLOWED CHANGE (must be clearly visible vs Image 1):
- New hairstyle and/or hair color (or bald/buzz/wig if specified in details) — hair may change; face must not
- Replace ALL visible clothing at shoulders/chest with a NEW premium outfit + ONE accessory
- Soft studio background
- Subtle realistic freshening ONLY: less under-eye bags, smoother rested skin, modest natural lift if relevant — still the SAME person, not 15+ years younger, not plastic

FRAMING: head-and-shoulders CLOSE-UP only, facing camera, photorealistic. NO full-body. Face sharp and clearly lit; catchlights in eyes.
Women/girls: bare face, NO makeup. Men: natural skin.
CRITICAL: Do NOT return the original photo unchanged. Hair, outfit, and subtle fresher skin must differ clearly from Image 1 — but the face must stay unmistakably the same person.
AFTER DETAILS: ${sanitizeEditPrompt(editPrompt || "")}`;
      ...
      const generateLookPair = async (
        ...
        try {
          ...
          const a = await generateImageWithFlux(afterPromptWrap(afterSrc), referenceImageBase64, mimeType, { quality: "basic" });
        ...
        const pair = {
          ...
          afterNote: look.afterNote
            || "Справа — ориентир: новая причёска, лучшая одежда и лицо после рекомендуемых процедур или сильного ухода. Это не гарантия; решение за вами и специалистом.",
          imageClose,
          imageAfter,
          imageFull: null,
          imageError,
