...
/** Восстановить заказы по телефону (другой телефон / очистили браузер). */
async function restoreOrdersByPhone(phoneRaw: string): Promise<{ ok: boolean; count: number; error?: string }> {
  ...
  if (!phone) return { ok: false, count: 0, error: "Укажите телефон в формате +7 XXX XXX-XX-XX" };
  try {
...
const PaymentModal = ({ isOpen, tier, onPaid, onClose }: {
  ...
  useEffect(() => {
    ...
    if (!phone && (tier === "standard" || tier === "premium")) {
      setLoading(false);
      alert("Укажите номер в окне тарифов — так заказ не потеряется, если связь оборвётся");
      onClose();
      return;
    }
...
const PricingModal = ({ isOpen, onClose, onPaid, onNailsUnlocked, userName, initialTier, prices }: {
  ...
  const [phoneInput, setPhoneInput] = useState(() => {
    ...
    return saved ? displayPhone(saved) : "+7 ";
  });
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setPhoneError("");
      const saved = getSavedPhone();
      setPhoneInput(saved ? displayPhone(saved) : "+7 ");
    }
  }, [isOpen, initialTier]);
  ...
  const requirePhone = (): string | null => {
    ...
    if (!phone) {
      setPhoneError("Укажите номер — если связь оборвётся, без него не вернём образы");
      return null;
    }
    setPhoneError("");
    savePhone(phone);
      ...
      >
        ...
        >
          ...
          <div className="p-5 md:p-8 md:p-10">
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-6">
              <button onClick={() => setSelectedTier("standard")}
                ...
                <ul className="text-sm space-y-1.5">
                  ...
                  <li className="text-charcoal/60">✓ Результат хранится 30 дней</li>
                </ul>
              </button>
              ...
              <button onClick={() => setSelectedTier("premium")}
                ...
                <ul className={`text-sm space-y-1.5 ${selectedTier === "premium" ? "text-ivory/70" : "text-charcoal/60"}`}>
                  ...
                  <li>✓ Результат хранится 30 дней</li>
                </ul>
              </button>
            </div>
            ...
            <label className="block mb-4 max-w-md mx-auto">
              <span className="block text-sm font-medium text-charcoal mb-1.5 text-center">
                Номер — страховка заказа, не регистрация
              </span>
              <input
                type="tel"
                inputMode="tel"
                ...
                className={`w-full px-4 py-3 rounded-xl border text-sm text-center bg-white focus:outline-none ${
                  phoneError ? "border-red-300 focus:border-red-400" : "border-charcoal/20 focus:border-gold"
                }`}
              />
              {phoneError ? (
                <p className="text-red-500 text-xs text-center mt-1.5">{phoneError}</p>
              ) : (
                <p className="text-charcoal/50 text-xs text-center mt-1.5 leading-relaxed">
                  Генерация идёт несколько минут. Если интернет моргнёт или страница закроется после оплаты — откроете образы и ссылки на одежду по этому номеру. Не звоним и рекламу не шлём.
                </p>
              )}
            </label>
...
const MyLooksModal = ({ isOpen, onClose, onOpenOrder, onClearAll, onOrderAgain }: {
  ...
  const [linkedPhone, setLinkedPhone] = useState(getSavedPhone);
  ...
  useEffect(() => {
    ...
    // Авто-восстановление, если телефон уже сохранён
    if (saved && fresh.length === 0) {
      restoreOrdersByPhone(saved).then((r) => {
        if (r.ok) {
  ...
  const handleRestore = async () => {
    ...
    setRestoring(false);
    if (!r.ok) {
      setRestoreMsg(r.error || "Не удалось найти заказы");
      return;
    }
    setLinkedPhone(getSavedPhone());
    const next = getMyOrders();
    setOrders(next);
    refreshOrdersMeta(next);
    setRestoreMsg(r.count > 0
      ? `Готово: найдено ${r.count} заказ${r.count === 1 ? "" : r.count < 5 ? "а" : "ов"}`
      : "По этому номеру пока нет заказов. После оплаты они появятся здесь.");
  };

  const handleClearAll = () => {
    if (confirm("Удалить все сохранённые образы из списка на этом устройстве? Оплата не вернётся — список можно снова открыть по телефону.")) {
      clearMyOrders();
      setOrders([]);
      onClearAll();
    }
        ...
        >
          ...
          <div className="p-6 md:p-8">
            ...
            <p className="text-[13px] md:text-sm text-charcoal/60 mb-4">
              Если оплатили, а страница закрылась или пропал интернет — найдите заказ по тому же номеру. Образы хранятся 30 дней.
            </p>

            <div className="rounded-2xl border border-charcoal/10 bg-white/60 p-4 mb-5">
              <p className="text-xs font-medium text-charcoal/70 mb-2">Найти заказ по номеру</p>
              ...
              {linkedPhone && (
                <p className="text-[11px] text-charcoal/45 mt-2 text-center">
                  Сохранён номер: {displayPhone(linkedPhone)}
                </p>
              )}
              {restoreMsg && (
            ...
            {orders.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-charcoal/40 text-xs mt-2">Введите тот же номер, что при оплате — или закажите новый образ.</p>
              </div>
            ) : (
              <>
...
const StylizeModal = ({ isOpen, onClose, userName, tier, orderPaymentId, onToast, onNewLooks, recoveredResult, onRecoveredResultShown, onOpenLightbox }: { isOpen: boolean; onClose: () => void; userName: string; tier: Tier; orderPaymentId?: string; onToast: (msg: string, type: 'success'|'error'|'info') => void; onNewLooks: () => void; recoveredResult?: any; onRecoveredResultShown?: () => void; onOpenLightbox?: (state: LightboxState) => void }) => {
  ...
  const [season, setSeason] = useState<"" | "зима" | "весна" | "лето" | "осень">("");
  ...
  const handleUpload = async () => {
    ...
    if (!season) { setErrorMsg("Выберите сезон — от него зависят образы и верхняя одежда."); return; }
    ...
    try {
      ...
      const seasonText = season
        ? `Сезон: ${season}. Все образы строго под ${season}: ткани, слои, верхняя одежда (пиджак/куртка/пальто/тренч по сезону) обязательны в каждом образе отдельной позицией со ссылкой для покупки.`
        : "";
      const fullWishes = [seasonText, occasionText, wishes].filter(Boolean).join(". ");
      ...
      >
        ...
        >
          ...
          <div className="p-5 md:p-8 lg:p-12 relative">
            ...
            <p className="text-charcoal/60 mb-4">{tier === "standard" ? "Загрузите фото, укажите рост и вес — стилист создаст 3 образа специально для вас." : "Загрузите до 3 фото, укажите параметры, и наш ИИ подберет идеальный гардероб."}</p>
              ...
              <div className="flex flex-col items-center">
                ...
                {!loadingState && (
                  <div className="w-full max-w-md mb-4 bg-charcoal/[0.04] border border-charcoal/10 rounded-xl p-3 text-xs text-charcoal/60 leading-relaxed">
                    Если связь слабая или включён VPN — лучше выключить VPN перед загрузкой фото. Если страница закроется, зайдите через 10 минут в «Мои образы»: заказ будет на месте.
                  </div>
                )}
                ...
                {/* Сезон — влияет на верхнюю одежду и ткани */}
                <div className="w-full max-w-md mb-4">
                  <label className="block text-sm font-medium text-charcoal/70 mb-2">
                    Сезон <span className="text-charcoal/40 font-normal">(обязательно)</span>
                  </label>
                  <p className="text-xs text-charcoal/50 mb-2">От сезона зависят ткани и верхняя одежда — офис зимой и летом это разные образы.</p>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {([
                      { label: "❄️ Зима", value: "зима" as const },
                      { label: "🌱 Весна", value: "весна" as const },
                      { label: "☀️ Лето", value: "лето" as const },
                      { label: "🍂 Осень", value: "осень" as const },
                    ]).map(({ label, value }) => (
                      <button
                        key={value}
                        type="button"
                              ...
                              <button
                                ...
                                onClick={async (e) => {
                                  ...
                                  const pid =
                                    ...
                                    || "";
                                  if (!pid) {
                                    onToast("Не найден номер заказа. Откройте этот заказ через «Мои образы» и повторите.", "error");
                                    return;
                                  }
...
const HOME_FAQ: { q: string; a: string }[] = [
  ...
  {
    q: "Зачем оставлять номер телефона?",
    a: "Чтобы ваш образ не потерялся по дороге. Генерация идёт несколько минут: интернет на телефоне может моргнуть, вкладка закрыться, банк открыться в другом окне.\n\nНомер — как номер гардероба в театре. По нему в «Мои образы» вы забираете фото и ссылки на вещи. Это не регистрация. Звонков, смс и рекламы нет.",
  },
  {
    q: "Вы будете звонить или присылать рекламу?",
    a: "Нет. По номеру не звоним и рассылок не делаем. Он только как ключ к заказу, если связь оборвалась или вы зашли с другого телефона.",
  },
  {
    q: "Что делать, если пропал интернет или я случайно вышел с сайта?",
    a: "Не платите второй раз. Заказ уже на сервере.\n\nЕсли заказывали образы (Стандарт или Премиум):\nШаг 1. Снова откройте stilist-ai.ru.\nШаг 2. Вверху нажмите «Мои образы».\nШаг 3. Введите тот же номер телефона, что указывали при оплате, и нажмите «Найти».\nШаг 4. Откройте нужный заказ. Если генерация ещё шла — подождите 10 минут и нажмите «Найти» ещё раз. Результат хранится 30 дней.\n\nЕсли заказывали «Причёска и уход»:\nШаг 1. Снова откройте сайт.\nШаг 2. Нажмите «Причёска и уход».\nШаг 3. Если оплата уже прошла, окно само подхватит заказ. Если нет — откройте «Мои образы» с тем же номером или напишите нам, не оплачивая заново.\n\nЕсли оплатили базу ногтей на месяц:\nШаг 1. Снова откройте сайт.\nШаг 2. Нажмите «Подобрать ногти».\nШаг 3. База должна открыться сама. Если каталог закрыт — не платите снова, напишите нам: внизу сайта почта и телефон.",
  },
  {
    q: "Оплатил, деньги списались, а образов нет. Что нажать?",
    a: "Шаг 1. Не нажимайте «Оплатить» повторно.\nШаг 2. Вверху сайта откройте «Мои образы».\nШаг 3. Введите номер, который был при оплате → «Найти».\nШаг 4. Если списка ещё нет — подождите 10 минут и повторите поиск. Генерация занимает 3–10 минут.\nШаг 5. Если через 15–20 минут заказа нет — напишите на почту gesper2004@mail.ru или по телефону 8 958 848-13-13, укажите время оплаты и номер. Мы найдём платёж.",
  },
  {
    q: "Как открыть готовый заказ в «Мои образы»?",
    a: "Шаг 1. Нажмите «Мои образы» вверху страницы (на телефоне — в меню).\nШаг 2. Введите номер в формате +7 … и нажмите «Найти».\nШаг 3. Выберите заказ в списке — откроются картинки, описания и ссылки на вещи.\n\nТак можно зайти с другого телефона или после очистки браузера. Хранение — 30 дней.",
  },
  {
    q: "Нужна ли регистрация, почта или пароль?",
    a: "Нет. Аккаунт создавать не нужно. Для образов достаточно номера телефона, чтобы потом найти заказ. Имя — по желанию.",
  },
  {
    q: "Что я получаю на тарифе Стандарт (100 ₽)? Как заказать?",
    a: "За 100 ₽ вы видите себя в трёх образах, которых нет у всех в ленте. Стилист не копирует тренд «как у всех» — собирает лук под вашу внешность, цвет и фигуру.\n\nВ каждом образе — ваше лицо, список вещей со ссылками на маркетплейсы (такие же или очень похожие) и советы по грумингу. 30 дней в «Мои образы».\n\nКак заказать:\nШаг 1. «Начать преображение» или тариф Стандарт.\nШаг 2. Номер телефона и 100 ₽.\nШаг 3. Фото лица анфас, рост и вес.\nШаг 4. 3–7 минут.\nШаг 5. Смотрите, скачивайте, отправляйте. Закрыли сайт — «Мои образы» и тот же номер.",
  },
  {
    q: "Что я получаю на тарифе Премиум (200 ₽)? Как заказать?",
    a: "Премиум — чтобы на свидании, в клубе или на фотосессии вас не спутали ни с кем. До пяти образов на вашем лице и 22 повода: отдых и пляж, фотосессия, клуб, ресторан, свидание, свадьба, офис, вечеринка, путешествие, театр, выпускной, корпоратив, романтический ужин… Можно смешать: два на свидание, один в клуб, два на отдых.\n\nПишете бюджет, например 5 000 ₽ — стилист собирает лук в этих деньгах и даёт ссылки на маркетплейсы. Не «как у всех в этом сезоне», а ваш. Есть астро-разбор. До трёх фото — чтобы посадка была вашей, не шаблонной. 30 дней в «Мои образы».\n\nКак заказать:\nШаг 1. Тариф Премиум.\nШаг 2. Номер и 200 ₽.\nШаг 3. Фото, рост, вес, поводы и бюджет.\nШаг 4. 5–10 минут.\nШаг 5. Фото + покупки — на экране и в «Мои образы».",
  },
  {
  ...
  },
  {
    ...
    a: "Ваши фото, рост и вес — только для вашего заказа. Мы не публикуем их, не продаём и не передаём другим людям. Не используем в рекламе и не показываем другим клиентам. Готовые образы лежат 30 дней в «Мои образы», чтобы вы сами могли их открыть. Потом заказ с сайта уходит.",
  },
  {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const phone = getSavedPhone();
    if (phone) restoreOrdersByPhone(phone).catch(() => {});
  }, []);
        ...
        <div className="relative text-center max-w-4xl mx-auto">
          ...
          >
            ...
            <a href="#faq" className="inline-block mt-3 text-xs sm:text-sm text-gold/90 hover:text-gold underline underline-offset-4">
              Зачем телефон, сколько ждать и что если пропал интернет — ответы здесь
            </a>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-ivory/45 text-sm max-w-xl mx-auto mt-3 font-light">
              Телефон спрашиваем только как ключ к заказу: если связь оборвётся — образы и ссылки на одежду не пропадут. Звонков и рекламы нет.
            </p>
          </motion.div>
