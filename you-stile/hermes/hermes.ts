...
function brandLookPrompt(s: DigestStory): string {
  ...
  return withEditorialTitle(
    `photoreal fashion magazine editorial of this look: ${s.kicker}. ${String(s.line || "").slice(0, 220)}. ` +
      ...
      `If a bag is in the look: wear it ON THE SHOULDER — strap over the shoulder, bag at the hip. FORBIDDEN: bag hooked on the elbow, crook of the arm, dangling from the forearm, or held in the hand. ` +
...
/** Перегенерировать кадр Зои/сумки в последнем выпуске: сумка через плечо, затем перевыложить альбом. */
async function fixLastLookPhoto(): Promise<{ ok: boolean; reason?: string; title?: string; tgMessageId?: number }> {
  const log = loadLog();
  const last = [...log.posts].reverse().find((p) => p.tgMessageId && /Мода, о которой сейчас говорят/.test(String(p.text || "")));
  if (!last) return { ok: false, reason: "no-news-post" };
  ...
  const story: DigestStory = {
    ...
    how: "сумка через плечо, не на локте и не в руке",
    kind: "star",
    angle: "wardrobe",
    link: "",
  ...
  for (let g = 0; g < 4 && !okFile; g++) {
    try {
      const imgUrl = await generateImage(zoeSaintLaurentStreetPrompt());
      ...
      if (await photoMatchesStory(tryPath, story)) {
        ...
        console.warn(`[Hermes] zoe fix vision reject try ${g + 1}`);
      }
    } catch (e) {
      console.warn("[Hermes] zoe fix generate:", (e as Error).message.slice(0, 140));
    }
  }
  if (!okFile) return { ok: false, reason: "vision-reject", title: last.title };
  return rewriteLastNewsPost();
}
...
async function photoMatchesStory(filePath: string, story: DigestStory): Promise<boolean> {
  ...
  const expect =
    story.angle === "nails"
      ...
      : `человек ${story.name} НАДЕЛ этот наряд «${story.kicker}»: видно и лицо, и одежду/кампанию. YES как в эталоне Деми Ловато — она в том платье, о котором текст. NO если только лицо, только ноги, группа где наряд не читается, витрина, манекен, другой человек, логотип агентства без образа, бумага, скриншот.${bagRule}`;
  try {
    const r = await polza.chat.completions.create({
      model: VISION_MODEL,
      messages: [
          ...
          content: [
            {
              type: "text",
              text:
                `Сверь фото с описанием. Ответь JSON одной строкой: {"ok":true,"why":"кратко"} или {"ok":false,"why":"что не совпало"}.\n` +
                `На фото должно быть: ${expect}\n` +
                `ok=false если витрина, манекен, магазин, другой человек, дети, бумага, скриншот, логотип без образа, или сумка не так надета как в тексте.`,
            },
    ...
    try {
      ...
      return /^yes\b/i.test(ans) || /"ok"\s*:\s*true/i.test(ans);
    }
  } catch (e) {
...
async function publishNewsOnce(slot: DaySlotKind = "women"): Promise<{ ok: boolean; reason?: string; title?: string }> {
  ...
  for (let i = 0; i < informative.length; i++) {
    ...
    if (!shots.length) {
      ...
      if (house) {
        for (let g = 0; g < 3 && shots.length < 1; g++) {
          try {
            const imgUrl = await generateImage(brandLookPrompt({ ...story, name: house.name }));
            const dest = path.join(PUBLIC_HERMES_DIR, `${id}-p${i}-house-${g}.jpg`);
            await downloadToFile(imgUrl, dest);
            await takeShot(dest);
            if (shots.length) console.log(`[Hermes] house generate ${house.name}: ${story.name} try ${g + 1}`);
          } catch (e) {
  ...
  while (picked.length < 3 && padN < 5) {
    ...
    if (!extraPhoto) {
      for (let g = 0; g < 3 && !extraPhoto; g++) {
        try {
          const imgUrl = await generateImage(brandLookPrompt(s));
          const extraDest = path.join(PUBLIC_HERMES_DIR, `${id}-hero-extra-gen-${padN}-${g}.jpg`);
          await downloadToFile(imgUrl, extraDest);
          if (await photoMatchesStory(extraDest, s)) extraPhoto = extraDest;
          else console.warn(`[Hermes] vision reject generated pad ${s.name} try ${g + 1}`);
        } catch (e) {
...
async function main(): Promise<void> {
  ...
  if (fixLastLook) {
    const r = await fixLastLookPhoto();
    console.log(`[Hermes] fix-last-look | ok=${r.ok} | ${r.title || r.reason} | msg=${r.tgMessageId ?? "—"}`);
    return;
  }
  if (retryPending) {
