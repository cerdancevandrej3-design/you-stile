    ...
    try {
      ...
      if (!gotFinalResult) {
        ...
        if (recoverId) {
          ...
          setErrorMsg("Связь прервалась. Откройте «Мои образы» — заказ там, нажмите «Продолжить». Платить снова не нужно.");
        } else {
      ...
      if (/Ошибка AI|AI не смог|Превышено время|JSON parse/i.test(msg)) {
        ...
        setErrorMsg("Произошла ошибка. Откройте «Мои образы» и нажмите «Продолжить» — если заказ уже оплачен, повтор бесплатный.");
      }
    } finally {
      setLoadingState(null);
    }
              ...
              <div className="flex flex-col items-center">
                ...
                {!loadingState && (
                  <div className="w-full max-w-md mb-4 bg-charcoal/[0.04] border border-charcoal/10 rounded-xl p-3 text-xs text-charcoal/60 leading-relaxed">
                    Если связь слабая или включён VPN — лучше выключить VPN перед загрузкой фото. Если страница закроется или генерации нет — «Мои образы», код СТИЛЬ-…, кнопка «Продолжить». Платить снова не нужно.
                  </div>
                )}
...
const HOME_FAQ: { q: string; a: string }[] = [
  ...
  },
  {
    ...
    a: "Не платите второй раз. Заказ уже на сервере.\n\nОдежда (Стандарт / Премиум) и «Причёска и уход» — один и тот же путь:\nШаг 1. Снова откройте stilist-ai.ru.\nШаг 2. Вверху нажмите «Мои образы» (на телефоне — в меню).\nШаг 3. Введите код заказа, например СТИЛЬ-K7M2QX, и нажмите «Найти».\nШаг 4. Откройте нужную строку. Если генерации нет — «Продолжить». Платить снова не нужно. Хранится сутки.\n\nБаза ногтей на месяц:\nШаг 1. Снова откройте сайт.\nШаг 2. Нажмите «Подобрать ногти».\nШаг 3. Если каталог закрыт — напишите нам, не платите второй раз.",
  },
  {
    q: "Оплатил, а генерации нет. Что делать?",
    a: "Не нажимайте «Оплатить» снова. Деньги уже на заказе.\n\nШаг 1. Откройте stilist-ai.ru.\nШаг 2. Вверху нажмите «Мои образы» (на телефоне — в меню).\nШаг 3. Введите код СТИЛЬ-… и нажмите «Найти».\nШаг 4. В списке отдельно одежда («Стандарт» / «Премиум») и «Причёска и уход». Нажмите «Продолжить» или «Открыть».\nШаг 5. Если фото ещё не отправляли — загрузите и запустите подбор. Если окно оборвалось посередине — продолжите с того же заказа, повтор бесплатный.\n\nЧерез 20 минут в «Мои образы» пусто — напишите на почту gesper2004@mail.ru или 8 958 848-13-13, укажите время оплаты и код.",
  },
  {
    q: "Как открыть готовый заказ в «Мои образы»?",
    a: "Шаг 1. Нажмите «Мои образы» вверху страницы (на смартфоне — в меню).\nШаг 2. Введите код заказа, например СТИЛЬ-K7M2QX, и нажмите «Найти».\nШаг 3. «Стандарт» или «Премиум» — образы одежды. «Причёска и уход» — причёски и уход. Если генерации нет — «Продолжить», не новая оплата.\n\nТак открывается заказ с другого телефона или после очистки браузера. Хранение — сутки.",
  },
  {
  ...
  },
  {
    ...
    a: "Стандарт — обычно 2–4 минуты: три образа рисуются сразу. Премиум — обычно 4–7 минут, до пяти образов сразу. «Причёска и уход»: бесплатное сравнение — 1–2 минуты, полный пакет из трёх кадров — обычно 2–5 минут. На экране виден прогресс.\n\nВкладку лучше не закрывать специально. Если закрыли, пропал интернет или картинок нет — «Мои образы», код СТИЛЬ-…, «Продолжить». Платить снова не нужно.",
  },
  {
    q: "Почему лучше выключить VPN?",
    a: "VPN иногда обрывает длинную генерацию: деньги уже списаны, а картинки не успели дойти. Это совет, не запрет. Если VPN нужен — оставьте. При обрыве: «Мои образы» → код СТИЛЬ-… → «Продолжить», без новой оплаты.",
  },
  {
  ...
  },
  {
    ...
    a: "Картой на сайте, через защищённую оплату. После оплаты начинается генерация или открывается база ногтей.\n\nЕсли окно банка закрылось, деньги ушли, а картинок нет — не жмите «Оплатить» снова. Одежда и причёска: «Мои образы» и код СТИЛЬ-…, затем «Продолжить». Ногти: «Подобрать ногти». Двойное списание — напишите нам с временем платежа, разберёмся.",
  },
  {
  ...
  },
  {
    ...
    a: "Если окно ещё открыто — у готового образа нажмите «Повторить генерацию». Это не новая оплата. В «Причёска и уход» у кадра «после» тоже есть повтор.\n\nЕсли окно закрылось или генерации нет совсем — «Мои образы», код СТИЛЬ-…, «Продолжить». Проверьте, что фото было чётким и анфас. Не помогло — напишите на почту / телефон внизу сайта.",
  },
  {
    ...
    <section id="faq" className="py-10 md:py-16 px-4 md:px-6 bg-ivory border-b border-charcoal/5 overflow-x-hidden">
      <div className="max-w-3xl mx-auto">
        ...
        <p className="text-[13px] md:text-sm text-charcoal/55 text-center mb-6 md:mb-8 max-w-xl mx-auto leading-relaxed">
          Зачем быть не как все, что внутри тарифов и что делать, если генерации нет.
        </p>
      ...
      {showProcessing && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-charcoal/80 backdrop-blur-sm">
          <div className="bg-ivory rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl">
            ...
            <p className="text-charcoal/60 text-sm mb-6">Если образы уже собираются — подождите. Если картинок нет — откройте «Мои образы» и нажмите «Продолжить», платить снова не нужно.</p>
            <div className="flex flex-col gap-2">
              <button onClick={() => {
                ...
                setCurrentTier(pendingTier === "premium" ? "premium" : "standard");
                setIsModalOpen(true);
              }} className="px-6 py-3 rounded-full bg-gold text-charcoal font-medium text-sm">
                Продолжить заказ
              </button>
            ...
            {[
              ...
              },
              {
                ...
                features: [
                  ...
                  "Если генерации нет — заказ в «Мои образы» сутки",
                ],
                highlighted: false,
                badge: null as string | null,
              },
